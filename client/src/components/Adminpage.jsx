            ...
            <tbody>
              {users.map((user) => (
                ...
                <tr key={user._id} className="border-b border-gray-700 hover:bg-gray-750 transition">
                  ...
                  <td className="py-4 px-6 text-[#443627]">{(task.filter((tsk)=>tsk.id==user._id)).length}</td>
                  <td className="py-4 px-6 text-[#443627]">{(team.filter((tm)=>tm.createdBy==user._id)).length}</td>
